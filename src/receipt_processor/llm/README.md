...
Contents:
- `config.py`: environment-driven settings (`ENABLE_LLM`, `ENABLE_LLM_EXCEPTION_ASSIST`, `OPENROUTER_*`, `LLM_INPUT_MODE`, `LLM_MAX_FILE_BYTES`).
...
Behavior:
- Default settings enable LLM and exception assist (`ENABLE_LLM=true`, `ENABLE_LLM_EXCEPTION_ASSIST=true`) while preserving deterministic fallback on any LLM failure.
...
- In `text` mode (default), local text extraction (with OCR fallback checks) is used before LLM calls.
...
- In `file` mode, direct file input is used first, and files over `LLM_MAX_FILE_BYTES` automatically fall back to text mode.
- LLM request context includes filename hints, matched notes text, and sanitized statement excerpts from the input folder.
...
- Optional review assist is intentionally modest: it can only select from explicit review options; number-based choices always route to user review, and if assist abstains (not obvious) or returns invalid/ambiguous output, user review remains in control.
